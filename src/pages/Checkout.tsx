...
export function Checkout() {
  ...
  const ticketsCount = cart.reduce((acc, item) => acc + item.quantity, 0);
  ...
  if (paymentMethod === 'card') {
    ...
    isPaymentValid = cleanCardNumber.length === 16 && cardExpiry.length >= 5 && cardCvc.length >= 3;
  ...
  const getPurchasedItems = () => {
    return cart.map(item => {
      ...
      const cleanLocationId = item.location_id.replace('resale_', '');
      const loc = ev?.locations.find(l => l.id === cleanLocationId);

      return {
        event_id: item.event_id,
        event_name: ev?.name || 'Evento',
        category: ev?.category || 'Sin categoría',
        city: ev?.city || 'Sin ciudad',
        venue: ev?.venue || 'Sin venue',
        date: ev?.date || '',
        location_id: item.location_id,
        ticket_tier: loc?.name || 'Localidad',
        price: item.price,
        quantity: item.quantity,
        item_revenue: item.price * item.quantity,
        is_resale: item.location_id.startsWith('resale_')
      };
    });
  };

  const getCheckoutAnalyticsProperties = () => {
    ...
    return {
      ...
      checkout_layout: experiments.checkout_layout,
      buyer_role: user?.role || 'anonymous',
      is_authenticated: user?.role !== 'anonymous',
      event_ids: purchasedItems.map(item => item.event_id),
      event_names: purchasedItems.map(item => item.event_name),
      categories: purchasedItems.map(item => item.category),
      cities: purchasedItems.map(item => item.city),
      ticket_tiers: purchasedItems.map(item => item.ticket_tier),
  ...
  useEffect(() => {
    ...
    trackEvent('Checkout Step Viewed', {
      step,
      step_name: step === 1 ? 'personal_information' : 'payment',
      step_order: step,
      ...getCheckoutAnalyticsProperties()
    });
  ...
  const handleSimulatePayment = (success: boolean) => {
    ...
    setTimeout(() => {
      ...
      if (success) {
        ...
        const orderId = `ML-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        ...
        trackEvent('Payment Failed', {
          ...getCheckoutAnalyticsProperties(),
          error_type: 'demo_declined',
          failure_reason: 'bank_declined_demo'
        });
      }
    }, 1500);
  };
    ...
    return (
      <div className="max-w-2xl mx-auto py-24 text-center space-y-6 animate-in fade-in zoom-in-95">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 my-8 text-left max-w-sm mx-auto">
          <h3 className="text-sm font-bold uppercase tracking-widest mb-4 border-b border-white/10 pb-2">
            Resumen de compra
          </h3>
              ...
              <div key={i} className="flex justify-between border-b border-white/5 pb-2">
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-gray-400">
                    {item.locationName} x{item.quantity}
                  </span>
          ...
          <div className="space-y-2 text-sm bg-white/5 p-3 rounded-xl border border-white/5">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-[var(--color-accent-mint)]">{formatCurrency(total)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Entradas totales:</span>
              <span className="font-bold text-white">{ticketsCount}</span>
            </div>
          </div>
        </div>
      ...
      <div className="lg:col-span-2 space-y-8">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-gray-400 hover:text-white flex items-center gap-2 text-sm uppercase tracking-widest font-bold"
          >
          ...
          <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-xl flex items-start gap-3 animate-in slide-in-from-top-2">
            ...
            <div>
              ...
              <p className="text-xs text-red-200 mt-1">
                Tu banco ha declinado la transacción. Por favor intenta con otro medio de pago (Demo).
              </p>
            </div>
          </div>
        )}
        ...
        <div
          className={`bg-[var(--color-surface)] border border-white/5 rounded-3xl p-8 shadow-2xl ${
            step !== 1 && experiments.checkout_layout === 'A' ? 'opacity-50 pointer-events-none' : ''
          }`}
        >
          <div className="flex justify-between items-center mb-6">
            ...
            {step > 1 && experiments.checkout_layout === 'A' && (
              <button
                onClick={() => setStep(1)}
                className="text-[var(--color-accent-cyan)] text-xs font-bold uppercase tracking-widest"
              >
                Editar
              </button>
            )}
          ...
          {step === 1 && experiments.checkout_layout === 'A' && (
            <button
              ...
              className="mt-8 px-8 py-4 bg-[var(--color-accent-purple)] text-white rounded-xl font-bold text-xs uppercase tracking-widest w-full hover:bg-purple-600 transition-colors disabled:opacity-50 disabled:grayscale shadow-lg shadow-purple-500/20"
            >
              Continuar al Pago
            </button>
          )}
        ...
        <div
          className={`bg-[var(--color-surface)] border border-white/5 rounded-3xl p-8 shadow-2xl ${
            step !== 2 && experiments.checkout_layout === 'A' ? 'opacity-50 pointer-events-none' : ''
          }`}
        >
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            <button
              ...
              className={`p-4 border rounded-2xl flex flex-col items-center justify-center gap-2 transition-colors ${
                paymentMethod === 'card'
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm text-white outline-none focus:border-[var(--color-accent-cyan)] transition-colors"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-5">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="MM/YY"
                      className="w-full p-3 bg-white/5 border border-white/10 rounded-xl text-sm text-white outline-none focus:border-[var(--color-accent-cyan)] transition-colors text-center"
                    />
                  </div>
            ...
            {paymentMethod === 'pse' && (
              <div className="animate-in fade-in slide-in-from-left-4">
                ...
                <select
                  ...
                  className="w-full p-4 bg-white/5 border border-white/10 rounded-xl text-sm text-white outline-none focus:border-[var(--color-accent-cyan)] transition-colors appearance-none"
                >
                ...
                {['Nequi', 'Daviplata', 'PayPal'].map(w => (
                  <label
                    ...
                    className={`flex items-center justify-center p-4 border rounded-xl cursor-pointer transition-colors ${
                      walletType === w.toLowerCase()
                        ? 'border-[var(--color-accent-pink)] bg-[var(--color-accent-pink)]/10 text-white font-bold'
                  ...
                  >
                    <input
                      type="radio"
                      value={w.toLowerCase()}
                      checked={walletType === w.toLowerCase()}
        ...
        <div className="bg-[var(--color-surface)] border border-white/5 rounded-3xl p-6 sticky top-24 shadow-2xl relative overflow-hidden">
          ...
          <div className="border-t border-white/10 pt-6">
            <label className="flex items-center justify-between text-[10px] uppercase tracking-widest text-gray-400 font-bold mb-2">
              ...
              <span className="bg-white/10 text-[8px] px-1.5 py-0.5 rounded text-gray-300">
                Demo: MINDERS10
              </span>
            </label>
            ...
            {couponError && (
              <p className="text-red-400 text-[10px] font-bold tracking-wider uppercase mt-2">
                {couponError}
              </p>
            )}

            {discount > 0 && (
              <p className="text-[var(--color-accent-cyan)] text-[10px] font-bold tracking-wider uppercase mt-2">
